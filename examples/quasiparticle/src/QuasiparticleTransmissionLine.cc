...
ConstructTransmissionLine(G4RotationMatrix* pRot,
  ...
  //--------------------------------------------------------------------
  //Start with a base layer of aluminum into which our objects will fit.
  ...
  //volume, not passed into this code. We therefore add this to the list of the
  //toBeBoundariedVolumeList with info on what kind of boundary it must have.
  //Since this base layer is actually *fully* occupied by empty pad/TL layers
  //that come later, we will make it the same boundary as those (? What happens
  //there?)
