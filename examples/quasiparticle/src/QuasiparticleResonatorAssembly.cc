...
void QuasiparticleResonatorAssembly::
ConstructResonatorAssembly(G4RotationMatrix * pRot,
  ...
  for (size_t iV = 0; iV < fFundamentalVolumeList.size(); ++iV) {
    ...
    //ground plane one layer further up. In this case, we will have to deal
    //with that in the detectorConstruction file.
  }
...
MakeShuntCapacitorCross(G4String pName, G4LogicalVolume * log_baseAlLayer,
  ...
  G4Box * solid_shuntVertBlockEmpty =
    new G4Box("shuntVertBlockEmpty",0.5*dp_shuntVertBlockEmptyDimX,
              0.5*dp_shuntVertBlockEmptyDimY,0.5 * dp_shuntVertBlockEmptyDimZ);
  G4Box * solid_shuntHorizontalBlockEmpty =
    new G4Box("ShuntHorizontalBlockEmpty",0.5*dp_shuntHorizontalBlockEmptyDimX,
              0.5*dp_shuntHorizontalBlockEmptyDimY,
              0.5 * dp_shuntHorizontalBlockEmptyDimZ);
  G4UnionSolid * solid_shuntEmpty =
    new G4UnionSolid(shuntEmptyNameSolid,solid_shuntVertBlockEmpty,
                     solid_shuntHorizontalBlockEmpty,0,G4ThreeVector(0,0,0));

  G4LogicalVolume * log_shuntEmpty =
    new G4LogicalVolume(solid_shuntEmpty,air_mat,shuntEmptyNameLog);
  ...
  G4Box * solid_shuntVertBlockConductor =
    new G4Box("shuntVertBlockConductor",0.5 * dp_shuntVertBlockConductorDimX,
  ...
  //...and set boundaries with the existing shuntConductor empty. Since no
  //other sister volumes exist yet, it's just this one needed
...
// Make the resonator line
void QuasiparticleResonatorAssembly::
MakeResonatorLine(G4String pName, G4LogicalVolume * log_baseAlLayer,
                  G4LatticeManager * LM,
                  std::map<std::string,G4LatticeLogical*> logicalLatticeContainer,
                  std::map<std::string,G4CMPSurfaceProperty*> borderContainer) {
  ...
  //This will be made in two batches: one for "empty" space and one for
  //"conductor" space (the line itself). Each batch has the following elements
  ...
  //Some useful translations relative to the center of the plane in which all
  //of this is embedded. Bottom right corner relative to center of the plane
  G4ThreeVector brCornerOfBaseAlLayer(0.5*dp_resonatorAssemblyBaseAlDimX,
                                      -0.5*dp_resonatorAssemblyBaseAlDimY,0);
  ...
  G4Box * solid_tlCouplingEmpty =
    new G4Box(tlCouplingEmptyNameSolid,0.5 * dp_tlCouplingEmptyDimX,
              0.5 * dp_tlCouplingEmptyDimY,0.5 * dp_tlCouplingEmptyDimZ);
  G4LogicalVolume * log_tlCouplingEmpty =
    new G4LogicalVolume(solid_tlCouplingEmpty,air_mat,tlCouplingEmptyNameLog);
  ...
  //Boundaries set here: set to other existing pieces. Need to set boundary
  //between this and the base Nb layer. But may do that ond layer up using the
  //fundamental volume list.

  //Note that this and the conductor version below are exactly contacting the
  //edge of the base layer, which means they also need to be made to contact
  //the 2-levels-up ground plane (and not just the base layer). I think these
  //are the only ones for which that applies.
  ...
  G4Box * solid_tlCouplingConductor =
    new G4Box(tlCouplingConductorNameSolid,0.5 * dp_tlCouplingConductorDimX,
              0.5 *dp_tlCouplingConductorDimY,0.5 * dp_tlCouplingConductorDimZ);
  ...
  //...and set boundaries with the existing TL empty. Since no other sister
  //volumes exist yet, it's just this one needed
  G4String tlCouplingConductor_boundaryName1 = tlCouplingConductorName
    + "_AlVac";
  ...
  G4VPhysicalVolume * curve1Empty =
    new G4PVPlacement(0,curve1WrtBRCorner+brCornerOfBaseAlLayer,log_curve1Empty,
                      curve1EmptyName,log_baseAlLayer,false,0,true);
  ...
  //..but need to set boundaries with the tlCouplingEmpty. This boundary
  //probably doesn't actually need to exist because it's vac-vac, but
  //better to have it than to need it and not know that it is the thing
  //causing the problem.
  ...
  //...and set boundaries with the existing TL empty AND the
  //tlCouplingConductor (sibling volume).
  ...
  G4Tubs * solid_curve2Empty =
    new G4Tubs(curve2EmptyNameSolid,dp_resonatorAssemblyCurveSmallestRadius,
               ...
               + dp_tlCouplingEmptyDimY,dp_curveEmptyDimZ/2.0,0.*deg,90.*deg);
  ...
  //..but need to set boundaries with the curve1Empty. This boundary probably
  //doesn't actually need to exist because it's vac-vac, but better to have it
  //than to need it and not know that it is the thing causing the problem.
  ...
  G4Box * solid_shl1Empty = new G4Box(shl1EmptyNameSolid,0.5 * dp_shl1EmptyDimX,
                                      0.5 * dp_shl1EmptyDimY,
                                      0.5 * dp_shl1EmptyDimZ);
  G4LogicalVolume * log_shl1Empty = new G4LogicalVolume(solid_shl1Empty,air_mat,
                                                        shl1EmptyNameLog);
  ...
  //..but need to set boundaries with the curve2Empty. This boundary probably
  //doesn't actually need to exist because it's vac-vac, but better to have it
  //than to need it and not know that it is the thing causing the problem.
  ...
  G4Box * solid_shl1Conductor =
    new G4Box(shl1ConductorNameSolid,0.5 * dp_shl1ConductorDimX,
              0.5 * dp_shl1ConductorDimY,0.5 * dp_shl1ConductorDimZ);
  ...
  G4Tubs * solid_halfCircle1Empty =
    new G4Tubs(halfCircle1EmptyNameSolid,
               dp_resonatorAssemblyCurveSmallestRadius,
               dp_resonatorAssemblyCurveSmallestRadius
               + dp_tlCouplingEmptyDimY,dp_curveEmptyDimZ/2.0,90.*deg,180.*deg);
  ...
  //..but need to set boundaries with the last empty one. This boundary
  //probably doesn't actually need to exist because it's vac-vac, but better to
  //have it than to need it and not know that it is the thing causing the
  //problem.
  ...
  G4VPhysicalVolume * halfCircle1Conductor =
    new G4PVPlacement(0,G4ThreeVector(0,0,0),log_halfCircle1Conductor,
                      halfCircle1ConductorName,log_halfCircle1Empty,
                      false,0,true);
  ...
  //...and set boundaries with the existing empty (parent) AND the previous
  //conductor (sibling volume);
  G4String halfCircle1Conductor_boundaryName1 = halfCircle1ConductorName
    + "_AlVac";
  G4String halfCircle1Conductor_boundaryName2 = halfCircle1ConductorName
    + "_VacAl";
  G4String halfCircle1Conductor_boundaryName3 = halfCircle1ConductorName
    + "_AlAl1";
  ...
  G4ThreeVector shl2WrtBRCorner = halfCircle1WrtBRCorner
    + G4ThreeVector(0.5*dp_shl2EmptyDimX,dp_resonatorAssemblyCurveCentralRadius,
                    0);
  ...
  //..but need to set boundaries with the last empty one. This boundary
  //probably doesn't actually need to exist because it's vac-vac, but
  //better to have it than to need it and not know that it is the thing causing
  //the problem.
  ...
  G4Box * solid_shl2Conductor =
    new G4Box(shl2ConductorNameSolid,0.5 * dp_shl2ConductorDimX,
              0.5 * dp_shl2ConductorDimY,0.5 * dp_shl2ConductorDimZ);
  ...
  G4Tubs * solid_halfCircle2Empty =
    new G4Tubs(halfCircle2EmptyNameSolid,
               dp_resonatorAssemblyCurveSmallestRadius,
               dp_resonatorAssemblyCurveSmallestRadius
               + dp_tlCouplingEmptyDimY,dp_curveEmptyDimZ/2.0,270.*deg,
               180.*deg);
  G4LogicalVolume * log_halfCircle2Empty =
    new G4LogicalVolume(solid_halfCircle2Empty,air_mat,
                        halfCircle2EmptyNameLog);  
  ...
  G4VPhysicalVolume * halfCircle2Empty =
    new G4PVPlacement(0,halfCircle2WrtBRCorner+brCornerOfBaseAlLayer,
                      log_halfCircle2Empty,halfCircle2EmptyName,log_baseAlLayer,
                      false,0,true);
  ...
  //...and set boundaries with the existing empty (parent) AND the previous conductor (sibling volume);
  G4String halfCircle2Conductor_boundaryName1 = halfCircle2ConductorName
    + "_AlVac";
  G4String halfCircle2Conductor_boundaryName2 = halfCircle2ConductorName
    + "_VacAl";
  G4String halfCircle2Conductor_boundaryName3 = halfCircle2ConductorName
    + "_AlAl1";
  ...
  G4Box * solid_shl3Empty =
    new G4Box(shl3EmptyNameSolid,0.5 * dp_shl3EmptyDimX,0.5 * dp_shl3EmptyDimY,
  ...
  G4Box * solid_shl3Conductor =
    new G4Box(shl3ConductorNameSolid,0.5 * dp_shl3ConductorDimX,
              0.5 * dp_shl3ConductorDimY,0.5 * dp_shl3ConductorDimZ);
  ...
  G4LogicalVolume * log_halfCircle3Empty =
    new G4LogicalVolume(solid_halfCircle3Empty,air_mat,
                        halfCircle3EmptyNameLog);  
  ...
  new G4CMPLogicalBorderSurface(halfCircle3Empty_boundaryName1,
                                halfCircle3Empty, shl3Empty, VacVacBoundary);
  ...
  G4VPhysicalVolume * halfCircle3Conductor =
    new G4PVPlacement(0,G4ThreeVector(0,0,0),log_halfCircle3Conductor,
                      halfCircle3ConductorName,log_halfCircle3Empty,
                      false,0,true);
  ...
  //...and set boundaries with the existing empty (parent) AND the previous
  //conductor (sibling volume);
  G4String halfCircle3Conductor_boundaryName1 = halfCircle3ConductorName
    + "_AlVac";
  G4String halfCircle3Conductor_boundaryName2 = halfCircle3ConductorName
    + "_VacAl";
  G4String halfCircle3Conductor_boundaryName3 = halfCircle3ConductorName
    + "_AlAl1";
  ...
  G4Box * solid_shl4Empty =
    new G4Box(shl4EmptyNameSolid,0.5 * dp_shl4EmptyDimX,0.5 * dp_shl4EmptyDimY,
              0.5 * dp_shl4EmptyDimZ);
  G4LogicalVolume * log_shl4Empty =
    new G4LogicalVolume(solid_shl4Empty,air_mat,shl4EmptyNameLog);
  G4ThreeVector shl4WrtBRCorner =
    halfCircle3WrtBRCorner
    + G4ThreeVector(0.5*dp_shl4EmptyDimX,dp_resonatorAssemblyCurveCentralRadius,
                    0);
  ...
  G4Box * solid_shl4Conductor =
    new G4Box(shl4ConductorNameSolid,0.5 * dp_shl4ConductorDimX,
              0.5 * dp_shl4ConductorDimY,0.5 * dp_shl4ConductorDimZ);
  ...
  G4LogicalVolume * log_halfCircle4Empty
    = new G4LogicalVolume(solid_halfCircle4Empty,air_mat,
                          halfCircle4EmptyNameLog);  
  G4ThreeVector halfCircle4WrtBRCorner =
    shl4WrtBRCorner +
    G4ThreeVector(+0.5*dp_shl4EmptyDimX,dp_resonatorAssemblyCurveCentralRadius,
                  0.0);
  G4VPhysicalVolume * halfCircle4Empty
    = new G4PVPlacement(0,halfCircle4WrtBRCorner+brCornerOfBaseAlLayer,
                        log_halfCircle4Empty,halfCircle4EmptyName,
  ...
  //...and set boundaries with the existing empty (parent) AND the previous conductor (sibling volume);
  G4String halfCircle4Conductor_boundaryName1 = halfCircle4ConductorName
    + "_AlVac";
  G4String halfCircle4Conductor_boundaryName2 = halfCircle4ConductorName
    + "_VacAl";
  G4String halfCircle4Conductor_boundaryName3 = halfCircle4ConductorName
    + "_AlAl1";
  ...
  G4ThreeVector shl5WrtBRCorner = halfCircle4WrtBRCorner
    + G4ThreeVector(-0.5*dp_shl5EmptyDimX,
                    dp_resonatorAssemblyCurveCentralRadius,0);
  ...
  G4Box * solid_shl5Conductor =
    new G4Box(shl5ConductorNameSolid,0.5 * dp_shl5ConductorDimX,
              0.5 * dp_shl5ConductorDimY,0.5 * dp_shl5ConductorDimZ);
  ...
  G4Tubs * solid_halfCircle5Empty =
    new G4Tubs(halfCircle5EmptyNameSolid,
               dp_resonatorAssemblyCurveSmallestRadius,
               dp_resonatorAssemblyCurveSmallestRadius
               + dp_tlCouplingEmptyDimY,dp_curveEmptyDimZ/2.0,90.*deg,
               180.*deg);
  ...
  //...and set boundaries with the existing empty (parent) AND the previous
  //conductor (sibling volume);
  G4String halfCircle5Conductor_boundaryName1 = halfCircle5ConductorName
    + "_AlVac";
  G4String halfCircle5Conductor_boundaryName2 = halfCircle5ConductorName
    + "_VacAl";
  G4String halfCircle5Conductor_boundaryName3 = halfCircle5ConductorName
    + "_AlAl1";
  ...
  G4Box * solid_shl6Empty =
    new G4Box(shl6EmptyNameSolid,0.5 * dp_shl6EmptyDimX,
              0.5 * dp_shl6EmptyDimY,0.5 * dp_shl6EmptyDimZ);
  ...
  G4ThreeVector shl6WrtBRCorner = halfCircle5WrtBRCorner
    + G4ThreeVector(0.5*dp_shl6EmptyDimX,dp_resonatorAssemblyCurveCentralRadius,
                    0);
  ...
  G4Box * solid_shl6Conductor =
    new G4Box(shl6ConductorNameSolid,0.5 * dp_shl6ConductorDimX,
              0.5 * dp_shl6ConductorDimY,0.5 * dp_shl6ConductorDimZ);
  ...
  G4LogicalVolume * log_halfCircle6Empty =
    new G4LogicalVolume(solid_halfCircle6Empty,air_mat,
                        halfCircle6EmptyNameLog);  
  ...
  G4VPhysicalVolume * halfCircle6Conductor =
    new G4PVPlacement(0,G4ThreeVector(0,0,0),log_halfCircle6Conductor,
                      halfCircle6ConductorName,log_halfCircle6Empty,
                      false,0,true);
  ...
  //...and set boundaries with the existing empty (parent) AND the previous conductor (sibling volume);
  G4String halfCircle6Conductor_boundaryName1 = halfCircle6ConductorName
    + "_AlVac";
  G4String halfCircle6Conductor_boundaryName2 = halfCircle6ConductorName
    + "_VacAl";
  G4String halfCircle6Conductor_boundaryName3 = halfCircle6ConductorName
    + "_AlAl1";
  ...
  G4Box * solid_shl7Empty =
    new G4Box(shl7EmptyNameSolid,0.5 * dp_shl7EmptyDimX,0.5 * dp_shl7EmptyDimY,
              0.5 * dp_shl7EmptyDimZ);
  G4LogicalVolume * log_shl7Empty =
    new G4LogicalVolume(solid_shl7Empty,air_mat,shl7EmptyNameLog);
  G4ThreeVector shl7WrtBRCorner = halfCircle6WrtBRCorner +
    G4ThreeVector(-0.5*dp_shl7EmptyDimX,dp_resonatorAssemblyCurveCentralRadius,
                  0);
  ...
  //..but need to set boundaries with the last empty one. This boundary probably
  //doesn't actually need to exist because it's vac-vac, but
  //better to have it than to need it and not know that it is the thing causing
  //the problem.
  ...
  //..but need to set boundaries with the last empty one, i.e. svl1. This
  //boundary probably doesn't actually need to exist because it's vac-vac, but
  //better to have it than to need it and not know that it is the thing causing
  //the problem.
  ...
  G4String shuntCouplerConductorNameSolid = shuntCouplerConductorName
    + "_solid";
  ...
  G4VPhysicalVolume * shuntCouplerConductor =
    new G4PVPlacement(0,G4ThreeVector(0,0,0),log_shuntCouplerConductor,
                      shuntCouplerConductorName,log_shuntCouplerEmpty,
                      false,0,true);
  ...
  //...and set boundaries with the existing empty (parent) AND the previous conductor (sibling volume);
  G4String shuntCouplerConductor_boundaryName1 = shuntCouplerConductorName
    + "_AlVac";
  G4String shuntCouplerConductor_boundaryName2 = shuntCouplerConductorName
    + "_VacAl";
  G4String shuntCouplerConductor_boundaryName3 = shuntCouplerConductorName
    + "_AlAl1";
  ...
  //At this point I should have boundaries between all things internal to
  //the CPW and themselves, and to the CPW gap.
  //I don't yet have anything in the z direction, or the gaps to the outer
  //ground plane -- will need to loop through
