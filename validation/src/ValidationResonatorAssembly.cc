...
// Default Constructor
ValidationResonatorAssembly::ValidationResonatorAssembly() {  
}
...
MakeResonatorLine(G4String pName,
  ...
  //between this and the base Nb layer. But may do that one layer up using the
  ...
  //No need to construct lattice but need to set boundaries with the
  //tlCouplingEmpty. This boundary probably doesn't actually need to exist
  //because it's vac-vac, but better to have it than to need it and not know
  //that it is the thing causing the problem.
  ...
  //No need to construct lattice but need to set boundaries with the
  //curve1Empty. This boundary probably doesn't actually need to exist because
  //it's vac-vac, but better to have it than to need it and not know that it
  //is the thing causing the problem.
  ...
  //No need to construct lattice but need to set boundaries with the
  //curve2Empty. This boundary probably doesn't actually need to exist because
  //it's vac-vac, but better to have it than to need it and not know that it
  //is the thing causing the problem.
  ...
  //No need to construct latticebut need to set boundaries with the last empty
  //one. This boundary probably doesn't actually need to exist because it's
  //vac-vac, but better to have it than to need it and not know that it is the
  ...
  //No need to construct lattice but need to set boundaries with the last
  //empty one. This boundary probably doesn't actually need to exist because
  //it's vac-vac, but better to have it than to need it and not know that it
  //is the thing causing the problem.
  ...
  //No need to construct lattice but need to set boundaries with the last empty
  //one. This boundary probably doesn't actually need to exist because it's
  //vac-vac, but better to have it than to need it and not know that it is the
  //thing causing the problem.
  ...
  //No need to construct lattice but need to set boundaries with the last
  //empty one. This boundary probably doesn't actually need to exist because
  //it's vac-vac, but better to have it than to need it and not know that it is
  //the thing causing the problem.
  ...
  //No need to construct lattice but need to set boundaries with the last
  //empty one, i.e. svl1. This boundary probably doesn't actually need to
  //exist because it's vac-vac, but better to have it than to need it and not
  //know that it is the thing causing the problem.
  ...
  //CPW and themselves, and to the CPW gap. I don't yet have anything in the z
  //direction, or the gaps to the outer ground plane -- will need to loop
  //through fundamental volume list and do this in above function.
  
}
  

std::vector<std::tuple<std::string,G4String,G4VPhysicalVolume*> >
ValidationResonatorAssembly::GetListOfAllFundamentalSubVolumes() {
