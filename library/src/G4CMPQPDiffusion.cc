...
G4CMPQPDiffusion::G4CMPQPDiffusion(const G4String& name,
  ...
  SetProcessSubType(fQPDiffusion);
  
  //This time step will be overwritten by the step-limiting length
  //(discrete process GPIL race)
  fTimeStep = 0;
  fPathLength =  0.0;
...
void G4CMPQPDiffusion::StartTracking(G4Track* track) {
  
  //Need to do this here because if we do it in the constructor, that runs
  //before the ConfigManager instance is defined in the current structure
  //of the examples
...
// This function returns a "diffusion-folded" path length, to be compared with
// Transportation and MFPs of other, potentially race-winning discrete GPIL
// processes.
...
AlongStepGetPhysicalInteractionLength(const G4Track& track,
  ...
  //GPIL race. Set the path length and pre-diffusion path length to the length
  ...
  //Get some track and SC properties. The diffusion constant and other
  //superconductor-specific information are accessed via the SCUtils class
  //that this inherits from. No need to actually grab it from somewhere:
  //it's already a data member
  ...
  if ((energy >= fGapEnergy) && (fDn > 0)) { 
    ...
    *selection = NotCandidateForSelection;
    ...
    //path length. If the currentMinimalStep matches the fTimeStepToBoundary,
    //it means that the winning discrete GPIL process is THIS process's
    //discrete bit. In this case, the diffusion displacement distance is
    ...
    if ((fabs(currentMinimalStep/velocity - fTimeStepToBoundary) <
      ...
      //trigger. Here we need to compute a radius corresponding to that
      //(smaller) elapsed time. However, the radius *must* be less than the
      //boundary-limited radius because otherwise the particle would have passed
      //through that radius, implying that the boundary-limited scenario
      //*should* have won. To make sure this is therefore self-consistent, we
      //will draw a radius from a gaussian corresponding to the (smaller)
      //elapsed time and require the drawn radius to be smaller than the
      //boundary distance. NB: this makes qualitative sense but may be a place
      //where we bias things on accident. May want to think through this logic
      //a bit more.
...
//that we pick a path length that is close to the 2D safety, with some
//restrictions.
//1. First, we want to ensure that if we're directing at the boundary,
//   we don't push tracks so close that they're within a hardFloor distance
//   of the boundary, since this may complicate boundary-finding and
//   direction-finding.
//2. Second, we want to make sure that we otherwise launch pretty much the
//   full 2Dsafety.
//The combination of these conditions defines what we call a "goldilocks zone"
G4double G4CMPQPDiffusion::ComputePathLengthInGoldilocksZone() {
  
  //If we're pointed right at the wall (along the safety), then we should go
  //no farther than the2DSafety - hardFloorBoundaryScale. If we're not pointed
  //right along the safety, then the distance to wall is longer anyway, so I
  //don't think we need to compensate for changing angles? To minimize errors
  //in timing, let's get more-or-less as close as we can to that hard floor
  //boundary scale.
  G4double thePathLength = f2DSafety -
    (fHardFloorBoundaryScale)*2*fBoundaryFudgeFactor;

  //Adding a bit of buffer (2*fBoundaryFudgeFactor) for safety. Btw,
  //this safety factor is intimately linked to the angular granularity --
  //the more granular it is, the safer we are here. But if we're at
...
PostStepGetPhysicalInteractionLength( const G4Track& track,
  ...
  //one (since it's called here in the base class). This is all just to get
  //the SCUtils updated at the beginning of the step calculus. Doing it in MFP
  //because that's where it's "usually" done with all of the other classes
  //(even though it doesn't really matter which step it's called in here)
  double mfp = GetMeanFreePath(track, previousStepSize, condition);

  //Reset this here so that if PostStepDoIt doesn't run, we can still reset.
  fNeedSweptSafetyInGetMFP = false; 
  return mfp;
}
...
G4VParticleChange* G4CMPQPDiffusion::AlongStepDoIt(const G4Track& track,
  ...
  //Limited by transport, needs to be overwritten.
  ...
  //Here, this occurs during a turnaround step where we want to set step
  ...
  if (!isActive) {
    ...
    //Have to stratify by three cases: off-boundary, on-boundary, and
    //on-boundary-and-stuck
    //Case 1: off-boundary
    ...
    if (!fTrackOnBoundary) {
      ...
      //Define a tolerance for the dot product of the emerging vector and the
      //surface norm, and then get the surface norm (here, the momentum dir
      //if the boundary interactions are handled correctly.)

      //Want to generate our points such that we start from the norm and
      //work outwards
      ...
      //vector should actually just be the surface normal in the direction of
    ...
    if (nextStepLength == kInfinity) { 
      ...
      //   one set here
      //2. When we hit the wall, we set the time of impact to be consistent with
      //   diffusion, so that the above code (without the "relocate" bit) still
      //   applies.
      //We're on a surface and may be confused


      //If this notes that the step status is not boundary limited, then it's
      //possible that in our current randomly-drawn direction, the distance to
      //the wall is shorter than what it was when the safety was calculated.
      //Because we use a "spam vectors" technique that doesn't cover a 2D
      //geometry fully in all angles, this may happen some fraction of the
      //time, especially with internal (convex) corners or sometimes around
      //curves. If this is true, try nudging our path length down by percent
...
// After we successfully check to see that the track isn't confused as to the
// distance to the boundary given its current location and nominal trajectory
// direction, go ahead and do the move. Wrapping this code into a dedicated
// function so that we can put it multiple places.
void G4CMPQPDiffusion::
PostCheckBulkTreatment(G4double stepTransportOnlyDeltaT) {
  ...
  //Since we are forced to use the pre-step point's velocity for propagation of
  //this step (and the pre-step point's velocity is the one we started with),
  ...
  //I think this needs to be set to the step-limiting *old, pre-diffusion* path
  //length, since other processes that aren't the step-limiting one will need
  //to subtract off a distance. That distance basically needs to be
...
// Handle near-boundary steps and update direction of the particle.
G4VParticleChange* 
G4CMPQPDiffusion::PostStepDoIt(const G4Track& track, const G4Step&) {
  ...
  //   Transportation will be the thing that runs PostStepDoIt, not this
  //   function gets set to true in the AlongStepGPIL
  //2. This shouldn't run if we are in a turnaround step
  //3. This *can* run if we are limited by this process while *starting* from
  //   a boundary, so determine if we're on a boundary. I think (?) that this
  //   should never get run if we *land* on a boundary, and during the
  //   turnaround step, I think this doesn't run anyway (?). It's either the
  ...
  //chance we'll want to nudge it slightly
  ...
  //Determine if we end up close to a boundary using Get2DSafety. Here it takes
  ...
  //There are infrequent but fundamentally possible cases where the 2D safety
  //is less than the "hard floor" length scale that we use to buffer our
  //various volume check length scales and our direction-to-boundary finding.
  //If this is the case, then we generate a random direction and bump the
  ...
  if (the2DSafety < fHardFloorBoundaryScale) {
    ...
    while (1) {
      ...
      the2DSafety = testSafety;
      trackPosition = nudgedPosition;
      fSafetyHelper->ReLocateWithinVolume(nudgedPosition);	
      break;
    }
  }
  ...
  //try to be efficient and use the simple DistToIn functions (and are not
  //limited by kCarTolerance), while my directional-based checks to mother
  //volumes are limited by kCarTolerance and can be zero if they're below that
  //threshold (even if they're not ACTUALLY zero).(I think, that is -- hard to
  //suss these errors out.) Here, we try shifting the particle backwards along
  ...
  //directly into the boundary so that boundary processes can run. We will
  //therefore not randomize the direction of the next step. Instead, we need
  //to find the direction toward the boundary in order to figure out how to
  //angle the next vector. Note that an exception to this runs if we're
  ...
  if (the2DSafety < fSoftFloorBoundaryScale) {
    ...
    //inside of the daughter volume (see G4Box, G4Trd DistToIn for examples).
    //In this scenario, the FindDirectionToNearbyBoundary will throw
    //a flag and identify that something about the boundary calculation is
    //funky. In this case, we will need to recompute the 2D safety and demand
    //that if the safety is to a daughter, we compute it using the sweep method
    //(rather than the "fast" and sometimes wrong directionless  DistToIn).
    //We'll do this for the initial Get2DSafety (i.e. Get2DSafetyWithDirection)
    //, which should also return a direction. This is a bit of a nuclear option
    ...
    //^If we're not within epsilon, then just purely randomize. This is easy.
  ...
  //In any case, propose the final position. If no nudging has occurred to keep
...
//Using the information about the track and the safety computed at the track
//point, we can find a direction to the nearby boundary. Note if there are
//shifts performed in the PostStepDoIt to compensate for being within the
//fHardFloorBoundary length scale, the trackPosition used here is not going
//to be exactly equal to the track.GetPosition(). In this scenario, the safety
//we want to compute is going to be that from the trackPosition and not from
//the track.GetPosition(), since we'll be nudging our final track position
//there anyway.
...
FindDirectionToNearbyBoundary(const G4Track& track,
  ...
  //First, find the momentum direction. Since in the last step we went from
  //outside epsilon to inside epsilon, this track vector should be pointed
  //roughly toward the boundary, rather than away from it. Use the momentum
  //direction to find another point that's a smidge farther from the boundary
  //than the current track point. Here we assume that this point is still within
  //the original touchable. If that's not true, things will break here. But a
  //delta of 1nm should be fine for this.
  G4ThreeVector momDir = track.GetMomentumDirection();

  //Sanity checks:
  //1. We set a default distance for computing another step and using this to
  //   find the direction to the wall as 1 nm
  G4double deltaPath = fHardFloorBoundaryScale*0.1;
  ...
  //the2DSafety and shiftedPoint2DSafety. We can now use these points to
  //identify an angle between the current momDir and the surface normal. For
  //now we'll assume that this angle is in XY, but later we should come back
  //and fix this to be more plane-agnostic.
  G4double deltaDistToSurface = shiftedPoint2DSafety - the2DSafety;

  //Sometimes our preliminary get2Dsafety functions don't do perfect
  //calculation of the safeties due to the finite granularity of the search in
  //phi. This can occassionally cause the deltaDistToSurface to be slightly
  //larger than the deltaPath, which in principle should never happen. If this
  ...
  //1. Complicated geometries with corners, though this is unlikely given how
  //   small the deltaPath parameter is -- would be very unlucky to get this.
  //2. If the step started on a boundary and launch our track at a steep angle,
  //   and we never *leave* the nearby epsilon region. Here, to re-launch
  //   ourselves back to the boundary, the final direction should be -1*momDir,
  //   with the attempted rotations down below. So if deltaDistToSurface is
  ...
  //relative to the surface. So what we'll do is try rotating our momDir
  //vector by theta in both directions, stepping the point forward by each,
  //and re-finding safeties. This calculation is specific to the XY plane and
  ...
  //and make sure we set our "probe" vectors' magnitudes no larger than that.
  //In fact, we want our probe vectors are unlikely to enter the hard floor
  //boundary region. Since the shifted safety is computed using a step of
  //deltaPath (which can at most get us closer by a fraction of the hardfloor
  //boundary scale, we want to make sure that our new positions also don't
  ...
  //Need to handle an edge case, which happens when the two options' safeties
  ...
  if (option1Safety < option2Safety) {
    ...
    if (option2Safety == 0 && option1Safety == 0) {
      ...
      //reason and are NOT zero. After exploration, this looks like a result
      //of a small safety and a track that is pointed almost perfectly normal
      //to a surface, and I don't think there's an issue here.
      outputDir = option1;
    }   
  }

  //Do a penultimate check: move from the current point's location along the
  //output dir by half of the safety and recompute. If the resulting safety
  //is NOT reasonably about 50% of the previous safety, then we need to redo
  //this entire function with the swept daughter safety applied. What is
  //probably happening is that we're hitting a point where inaccuracies in G4
  //geometry DistanceToIns are problematic.
  G4double fractionalSafetyDifferenceThreshold = 0.1; //A biiiit arbitrary...
  ...
  //If we pass this and don't need to repeat based on this, do a final check
  //to make sure we're actually crossing a boundary from one volume into
...
//Check to see if our safety and output direction (i.e. direction to the
//boundary) tells us to cross a boundary that isn't really there but which
//only seems to be there because of the rapid, inaccurate way that G4
//distance-to-inside functions are calculated for various geometrical
...
CheckForPhantomBoundaryCrossings(G4ThreeVector trackPosition,
  ...
  if (originalOption1Safety == 0 && originalOption2Safety == 0) {
    ...
    //Due to the new hard floor formalism, this should never run.
    ...
    posOstensiblyOverBoundary =
      trackPosition + the2DSafety*outputDir*fBoundaryFudgeFactor;
  }
  

  //Check to see if the volume at the original position is still equal to the
...
//Calculates interaction length to be compared with other discrete processes.
...
G4double G4CMPQPDiffusion::GetMeanFreePath(const G4Track& track,
  ...
  //in the base G4VContinuousDiscrete class, but since we haven't set the
  //condition variable anywhere else, I think this ends up taking on a garbage
  //value unless we set it. Here we set it to not forced, which makes this
  //class's discrete bit compete on equal footing with the other discrete
  ...
  *condition = NotForced;
  ...
  //Figure out some basic info, including volumes at the current position +/-
  //epsilon. Using the epsilon as a fraction (10%) of the hard floor boundary
  //scale. I think we need to make sure that it's just much larger than the
  //picometer scale that G4 uses for tolerance checks. I've also noticed that
  ...
  //If the volume plus epsilon (in the direction of the step) and the current
  //volume are not the same (and if we're in a boundary-limited step), then
  ...
  if (currentVolPlusEps != track.GetVolume() && theStatus == fGeomBoundary) {
    ...
    //pushes us across a corner back into World, we'll have an issue. Hopefully
    //this only happens in very "badly-behaved" geometries (like 20-
    //pointed stars or something with highly acute internal corners), since the
    //momentum should in principle be the normal vector based on the boundary
    //physics.
  ...
  //into a volume with a lattice. Since we've updated the lattice at this
  //point, we should confirm that the current procUtils lattice is not null,
  ...
  if ((energy>=fGapEnergy) && (fDn)>0) { 
    ...
    //one where we're on a boundary. Boundary case: here, we're not in a
    ...
    if (fTrackOnBoundary) {
      ...
      //GPIL race with other processes in either case. First, establish the
      //surface norm. From the boundary process, this should just be the
      //momentum direction at the pre-step point (track point). Then check to
      //see if the QP is stuck
      G4ThreeVector surfaceNorm = track.GetMomentumDirection();      
    ...
    //established by the safety. For now we use something simple and wrong to
    //test, taking the 2D safety to be the sigma of the diffusion broadening
    //but need the true expression from Grebenkov
    G4double timeStepToBoundary =
      SampleTimeStepFromFirstPassageDistribution(the2DSafety);    
    ...
    //"full" distance that can be compared to the other discrete processes.
    //Since those use a characteristic time multipled by a velocity that is
    //quite high, we need to do the same.
    ...
    //that the only way that another process will win over this in the discrete
    //GPIL race is if the time to that process is less than the time to this
    //one.
...
//Special treatment of steps that are "small" (defined by a crossing point of
...
G4double G4CMPQPDiffusion::HandleVerySmallSteps(G4double thisMFP,
  ...
  //Since the time of first passage is sampled given a diffusion constant and
  //a distance (the2DSafety), and since the MFP is linearly related to that
  //TOFP via an artificially constant velocity, there may be a scenario in which
  //the MFP (i.e. diffusion-unfolded distance) occasionally ends up smaller
  //than the "straight-line" 2D safety. The important thing here is actually
  //the relationship between diffusion constant and velocity, which is I think
...
//This is an amended version of the logic block in G4CMPVProcess that allows us
//to update the lattice in procUtils and in SCUtils when it changes.
//Since we don't use a rateModel or other things explicitly here, it's not the
//exact same code, which is why I'm okayish with havine a separate function in
//this class.
G4bool G4CMPQPDiffusion::
...
void G4CMPQPDiffusion::UpdateSCAfterLatticeChange() {
  ...
  //class within the base of this. Also, handle the checking/updating of the
  //lookup tables to be used for each SC.
  this->LoadLatticeInfoIntoSCUtils(this->theLattice);
}


//Sample a time step for first passage through a 2D disk of radius the2DSafety.
//This is from the 2013 Grebenkov paper on efficient monte carlo techniques
//for diffusion. (arXiv:1304.7807, physics.comp-ph)
G4double G4CMPQPDiffusion::
SampleTimeStepFromFirstPassageDistribution(G4double the2DSafety) {
...
SampleDimensionlessTimeStepUsingAcceptanceRejectionTechnique() {
  ...
  //Hardcoded, but the max value of our calculated Grebenkov disk pdf for dSc/dt
  G4double minT = 0.001;
  G4double maxT = 3;
  G4double maxVal = 4.06615; 
  ...
  while (1) {
    ...
    double BesselJ1_evalAt_alpha00 = 0.519153; //Hardcoded
    double alpha00 = 2.4048; //Hardcoded
    double derivHigh =
      -2.0*alpha00/BesselJ1_evalAt_alpha00*exp(-1*alpha00*alpha00*sampleT);
    double derivLow =
...
//This looks at the storage objects and uses them to understand if our track
//hasn't really moved much in the last several boundary scatters. The outputs
//are whether a QP is stuck and then two vectors that give a span over which
//to search for a new 2D safety.
std::tuple<G4bool,G4ThreeVector,G4ThreeVector,G4ThreeVector,G4ThreeVector> G4CMPQPDiffusion::CheckForStuckQPs() {
  
  //For now, just check for stuck QPs in a corner geometry
  return CheckForStuckQPsInCorner();
}

//For a specific geometry where we are explicitly dealing with a corner (doesn't
//have to be 90 degrees, but it has to be discontinuous). General strategy:
std::tuple<G4bool,G4ThreeVector,G4ThreeVector,G4ThreeVector,G4ThreeVector> G4CMPQPDiffusion::CheckForStuckQPsInCorner() {
  ...
  //Find the most recent boundary history norm and position, and which volume
  //is in the "forward" direction
  G4double epsilonDisplacement = fHardFloorBoundaryScale*0.1;
  G4ThreeVector currentPosition =
    fBoundaryHistory[fBoundaryHistory.size()-1].first;
  G4ThreeVector currentNorm =
  ...
  for (int iB = 0; iB < ((int)fBoundaryHistory.size()); ++iB) {
    ...
    //tightly clustered the boundary hits are.
    ...
    //Now determine if the currentNorm is collinear with this historical norm.
    ...
    //Since we now have a norm that is not collinear, compute +/- eps
    //distances from this position along this norm, and determine if it's in
    //the same volume as the current norm's next volume. This should leave us
    //with a set of norms and points corresponding to the other positions that
    //point into the same volume as the "main" one
    G4ThreeVector plusEps = fBoundaryHistory[iB].first +
  ...
  //So we're boxed into a corner. Now check the good positions list to see if
  //there are more than one unique vector that points to this next
  ...
  //Now we should have a set of unique good norms, pointing into the volume of
...
void G4CMPQPDiffusion::UpdateBoundaryHistory(G4int trackID,
  ...
  if (trackID != fBoundaryHistoryTrackID) {
    ...
    //^If we're on the same particle, then push back our boundary history
    //vectors until they hit a max length, and then start kicking old boundary
...
//This takes the positions and norms from the "am I stuck" function and uses
//them to find the outgoing tangent vectors from a corner. Note that this
//function is not yet agnostic to the plane: it's only in XY for now. This
...
FindSurfaceTangentsForStuckQPEjection(G4ThreeVector norm1,
  ...
  //vectorial, and fully general. Here we use a prescription and notation in
  //songho.ca/math/line/line.html where we have two lines,
  //P1 + t * v and Q1 + s * u
  //where we take pos1 = P1, pos2 = Q, tangVect1 = v, tangVect2 = u.
  //We first compute the parameter t at which those two lines are equal to
  //each other:
  ...
  //Some edge cases have arisen where one of these has ended up as the zero
  //vector. I think this happens if the corner finding has spatial points
  //that are too close to each other? In any case, except and abort the event.
  //Update: I think this happens when we land too close to the corner of a
  //volume. If we do, then G4 (for some reason) likes to make the surface
  //normal at a 45 degree angle (for G4Box and kCarTolerance, for example)?
  //This causes all sorts of weird issues, like our initial surface tangents
  //being not collinear with our output surface tangents because, for example,
  //we're using points that truly span a 90 degree bend but are also assuming
  //G4-computed tangent vectors at those points, one of which is at a 45
  //degree angle. If any of this nonsense hits, just nuke the event for now.
  //We're unfortunately just going to run into this some fraction of the time...
  //Check that finalTangVect1 is collinear with at least one initial tangVect,
  //and that neither are zero.
