...
G4double G4CMPQPRadiatesPhononRate::Rate(const G4Track& aTrack) const {
  return Rate(aTrack,0);
}

// Phonon radiation rate is computed using energy and the G4SCUtils class,
// upon which this is based
G4double G4CMPQPRadiatesPhononRate::
Rate(const G4Track& aTrack, const G4LatticePhysical * theLat) const {

  //Debugging
  if (verboseLevel > 5) {
    G4cout << "-- G4CMPQPRadiatesPhononRate::Rate --" << G4endl;
  } 
  ...
  if (!useInputLat) {
    ...
    if (fMap_physicalLattice_NormalizedTauQPRadiatesPhononVsEnergy.count(theLat)
      ...
      G4Exception("G4CMPQPRadiatesPhononRate::Rate",
...
bool G4CMPQPRadiatesPhononRate::
    ...
    if (fGap0Energy==0 || fTau0_qp == DBL_MAX || fTcrit == 0 || fTeff == 0) {
      ...
      if (fGap0Energy==0 && fTau0_qp == DBL_MAX && fTcrit == 0 && fTeff == 0) {
        ...
        msg << "Noticed that in the rate calculation step for the QP "
            << "phononradiation process you've incorrectly defined or omitted "
            ...
            << "the phononRadiation physics correctly.";
    ...
    if (theLat->GetSCDelta0()==0 || theLat->GetSCTau0qp() == DBL_MAX ||
      ...
      if (theLat->GetSCDelta0()==0 && theLat->GetSCTau0qp() == DBL_MAX &&
        ...
        msg << "Noticed that in the rate calculation step for the QP "
            ...
            << "the phononRadiation physics correctly.";
        G4Exception("G4CMPQPRadiatesPhononRate::CheckToSeeSCParametersSet",
                    "QPRadiatesPhononRate002",JustWarning, msg);
        return false;
      }
    }
    return true;
...
// If we arrive in a new lattice, either compute the scattering rate and then
// update the lookup table, or update the lookup table using a computed rate
// curve that already exists in the map
void
G4CMPQPRadiatesPhononRate::UpdateLookupTable(const G4LatticePhysical * theLat) {
  ...
  if (!CheckLookupTableForLat(theLat)) {
    if (verboseLevel > 5) {
      G4cout << "Computing new lookup table for phonon radiation process, "
             << "lattice  name: " << theLat->GetLattice()->GetName() << G4endl;
    }
    fMap_physicalLattice_NormalizedTauQPRadiatesPhononVsEnergy.
      emplace(theLat,ComputeNormalizedTauQPRadiatesPhononVsEnergy());
    fCurrentNormalizedTauQPRadiatesPhononVsEnergy =
...
// Construct the lookup table for normalized tau for scattering vs phonon
// energy. Note that this is JUST for QPs radiating phonon at the moment
std::vector<std::vector<G4double> >
G4CMPQPRadiatesPhononRate::ComputeNormalizedTauQPRadiatesPhononVsEnergy() {
  ...
  for (int iB = 0; iB < fQPEnergyBins; ++iB) {
    ...
    //Removing Z0 since it's canceled later
    G4double prefactor
    ...
    //Again omitting b and Z0 since this is where they're divided out
